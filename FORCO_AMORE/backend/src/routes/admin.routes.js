...
const PLATFORM_PAYEE_NAME = process.env.PLATFORM_PAYEE_NAME || 'Forco Amore Platform';
...
// ── PATCH /admin/orders/:id/complete — Complete a Pay at Counter order ──
router.patch('/orders/:id/complete', async (req, res, next) => {
    try {
        const order = await transitionOrderStatus(req.params.id, {
            ...
            changedBy: req.user.id,
            changedByRole: ROLES.ADMIN,
            notes: 'Order paid at counter and completed.'
        });
