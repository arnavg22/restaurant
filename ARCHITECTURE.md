# 🍽️ Cafeteria Green — System Architecture
...
**Cafeteria Green** is a mobile-first food ordering platform for a locally-run restaurant operating in a corporate area. Office workers browse the menu, place orders, pay via UPI QR, and get food delivered to their office desk. The platform takes a **15% commission** on every order.
...
### Base URL: `https://api.cafeteriagrean.in/v1`
...
### WebSocket Events
```
Connection: ws://api.cafeteriagrean.in/ws?token=<access_token>
...
**Example:**
```
upi://pay?pa=cafeteriagrean@paytm&pn=Cafeteria%20Green&am=250.00&cu=INR&tn=Order%20GRN-20260606-0001&tr=GRN202606060001
```
...
|-------|------------------------------------|------------------------------------------|
| `pa`  | Payee VPA (UPI ID)                 | `cafeteriagrean@paytm` (your UPI handle) |
| `pn`  | Payee Name                         | `Cafeteria Green`                        |
...
function PaymentQR({ order }) {
    const upiLink = `upi://pay?pa=cafeteriagrean@paytm&pn=${encodeURIComponent('Cafeteria Green')}&am=${order.total_amount}&cu=INR&tn=${encodeURIComponent(`Order ${order.order_number}`)}&tr=${order.order_number}`;
...
services:
  ...
  api:
    build: ./backend
    environment:
      - DATABASE_URL=postgresql://cg_user:password@postgres:5432/cafeteria_green
      - REDIS_URL=redis://redis:6379
      - JWT_SECRET=<secret>
      - UPI_VPA=cafeteriagrean@paytm
      - UPI_PAYEE_NAME=Cafeteria Green
...
*Architecture designed for Cafeteria Green — a commission-based food ordering platform for corporate office delivery.*
